...
module hardware(type,preview=false) {
    ...
    if (type=="coupler") color("gold", preview ? 0.7 : 1) { cylinder(d=6, h=4); translate([0,0,4]) cylinder(d=8, h=5, $fn=6); translate([0,0,9]) color("white", preview ? 0.7 : 1) cylinder(d=10, h=4); }
